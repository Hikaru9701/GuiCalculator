...
void Calculation::formulaCalculator(const QString& formula)
{
    ...
    // 無意味な入力を正規化
    if (receivedFormula.isEmpty() || receivedFormula == "(" || receivedFormula == ")" || receivedFormula == "()" || receivedFormula == ")(") {
        receivedFormula = "0";
    }
    ...
    qDebug() << __FUNCTION__ << "result(double)" << QString::number(gAnsDouble, 'f', 13);
    
    // 結果処理
	gAnsString = QString::number(gAnsDouble, 'f', 13);                      // 小数点以下11位残す、文字列に変換
	qDebug() << __FUNCTION__ << "result(QString)1" << gAnsString;
//	double temp = gAnsString.toDouble();									// doubleに変換
	rx.setPattern("(\\.){0,1}0+$");											// 小数点以下最後の「0」を消すパターン
	gAnsString = gAnsString.replace(rx, "");
...
double Calculation::postfixCalculator(const QStringList& postfix)  
{
    ...
    for (int i = 0; i < postfix.size(); ++i) {
        ...
        if (element.size() == 1 &&
            ...
            || element.at(0) == "/")) {
            // スタックの一番上の数字をrにポップ
            double r = workStack.pop();

            // スタックの二番上の数字をlにポップ
            double l = workStack.pop();
            ...
            // 結果をstackに入れる
            workStack.push(calculationResult);
        }
        else {
            // 数字は直接的にstackに入れる
            workStack.push(element.toDouble());
        }
    }
